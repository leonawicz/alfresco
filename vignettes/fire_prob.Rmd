...
Using the JFSP project simulation outputs and Alaskan communities from the `snaplocs` package, select a few points of interest and estimate 
fire probabilities. The code below uses a random sample (`n = 100`) among all climate models, emissions scenarios, projected years 2014 - 2099,
simulation replicates, and fire management treatment levels. This yields 5,000 observations.

#### Sample communities

First sample 50 community point locations.
...
id <- places$loc
system.time( p <- purrr::map(1:n, ~point_probs(r = sample_files$scar[.x], xy, id, veg = sample_files$veg[.x])) %>% bind_rows() )
```
